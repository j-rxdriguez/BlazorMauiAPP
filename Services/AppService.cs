...
namespace LoginFlowInMauiBlazorApp.Services
{
    public class AppService : IAppService
    {
        ...
        public AppService(IHttpService http)
        {
            _http = http;
        }
        ...
        {
            var url = "https://localhost:7127/api/account/login";
            var request = await _http.Post(true, url, loginModel);
            if(request.IsSuccessStatusCode)
            {
                var stringObject = await request.Content.ReadAsStringAsync();
                var result = JsonConvert.DeserializeObject<LoginResponseDTO>(stringObject);
                return result;
            }
            return null;
        }
